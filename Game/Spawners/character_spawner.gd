...
func spawn_start_player_characters(blue_ship: Vector2i, orange_ship: Vector2i):
	for char_data in player_roster:
		...
		new_char.global_position = tilemap.get_global_pos(new_char.current_tile_coords)
		board_manager.register_playable_character(new_char)
		board_manager.set_character_formation_on_tile(new_char.current_tile_coords, new_char)
