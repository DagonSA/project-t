...
func update_tile_formation(map: Dictionary[Vector2i, Array], tile: Vector2i):
	...
	if team_count.size() == 1: 
		for i in char_count:
			...
			char.global_position = get_node("../L0_tilemap_data").get_global_pos(tile) + TileFormations.ONE_TEAM_FORMATION[char_count-1][i]
		...
		for i in blue_right_side.size():
			...
			blue_char.global_position = global_position + TileFormations.TWO_TEAM_FORMATION[blue_right_side.size() -1][i] + TileFormations.SIDE_BLUE_RIGHT_X
		for i in orange_left_side.size():
			var orange_char = orange_left_side[i]["char"]
			var global_position = tilemap_base_L0.get_global_pos(tile)
			orange_char.global_position = global_position + TileFormations.TWO_TEAM_FORMATION[orange_left_side.size() -1][i] + TileFormations.SIDE_ORANGE_LEFT_X
